...
const HistoryTable = () => {
        ...
        data = combinedDocs.map((doc) => {
          ...
          return {
            ...
            actionDate: formatDate(
              approvalData.approvedAt || approvalData.rejectedAt,
            ),
            action:
              approvalData.approved === false
                ? 'Rejected'
  ...
  const filteredData = dataSource.filter((item) => {
    ...
    const sourceForFilter =
      userProfile.role === 'Requester' ? item.status : item.action
    const matchesStatus = selectedStatus
      ? sourceForFilter === selectedStatus
      : true

    const matchesDateRange =
      selectedDateRange && selectedDateRange.length === 2
        ? dayjs(item.requestDate).isBetween(
            selectedDateRange[0],
            selectedDateRange[1],
            'day',
            '[]',
  ...
  const requesterColumns: ColumnsType<DataType> = [
    ...
    },
    {
      ...
      key: 'requestNo',
      align: 'center' as const,
      // You can create a common function for the formatting
      sorter: (a, b) => {
  ...
  return (
    <div>
        ...
        <Input.Group compact style={{ width: 'auto' }}>
          ...
          >
            {userProfile?.role === 'Requester' ? (
              ...
              Array.from(new Set(dataSource.map((item) => item.action))).map(
                (action) => (
      ...
      <Table
        columns={
          userProfile && userProfile.role === 'Requester'
            ? requesterColumns
            : actionColumns
        }
